...
|---|---|---|---|
| 波形播放 | 固定时间分页 | 播放头推进，页边界切页 | 阅图显示 |
| 静态 PSD | 用户提交的 4–120 s 区间 | 区间提交或固定页变化时重算 | 稳定频谱摘要 |
| 动态 PSD | 最近 5/10/20/30 s | 播放中每 1/2/5 s 重算 | 观察近期频谱变化 |
| Spectrogram | 选定的 4–120 s 区间 | 每个 4 s 窗以 1 s 步进形成时间 bin | 时间 × 频率变化 |

波形的屏幕时长不是动态 PSD 的分析窗口。两者可以同为 10 s，但属于独立配置。波形 Worker、擦除带、缩放和分页只改变展示，不改变后端分析输入。
