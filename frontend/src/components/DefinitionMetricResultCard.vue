...
const bandValues = computed(() => props.result.chart.kind === 'band_share' ? props.result.band_values ?? {} : {})
...
<template>
  <section class="definition-metric-result" aria-label="算法运行结果">
    ...
    <div v-if="result.chart.kind !== 'band_share'" class="definition-metric-value"><span>{{ result.output.label }}</span><strong>{{ result.output.value ?? '不可用' }}</strong><em>{{ unitDisplay(result.output.unit) }}</em></div>
    <div v-else class="definition-metric-value"><span>{{ result.output.label }}</span><strong>四频段占比</strong><em>比例</em></div>
    ...
    <div v-if="Object.keys(bandValues).length" data-testid="official-rbp-values" class="metric-input-chart">
      <strong>相对频段功率</strong><small>同一后端 PSD 的四频段功率分别除以 1–30 Hz 总功率。</small>
      <div v-for="band in ['delta', 'theta', 'alpha', 'beta']" :key="band" class="metric-input-bar-row"><span>{{ band[0].toUpperCase() + band.slice(1) }}</span><i><b :style="{ width: `${((bandValues[band] ?? 0) * 100)}%` }" /></i><em>{{ bandValues[band] === null ? '不可用' : `${((bandValues[band] ?? 0) * 100).toFixed(2)}%` }}</em></div>
    </div>
