...
const point = computed(() => points.value.at(-1) ?? null)
const isDynamic = computed(() => metric.value.mode === 'dynamic' || points.value.length > 0)
  ...
  <div class="modal-layer algorithm-metric-debug-layer" @click.self="emit('close')">
    <section class="algorithm-metric-debug-dialog" role="dialog" aria-modal="true" aria-label="算法调试台">
      ...
      <main class="algorithm-metric-debug-body">
        ...
        <section class="algorithm-debug-section"><h3>本次分析范围</h3><dl><dt>模式</dt><dd>{{ isDynamic ? '动态算法' : '静态算法' }}</dd><dt v-if="isDynamic">波形播放位置</dt><dd v-if="isDynamic">{{ props.playbackPositionS === undefined ? '—' : `${props.playbackPositionS.toFixed(3)} s` }}</dd><dt>实际分析区间</dt><dd>{{ actualRange.start_s.toFixed(3) }}–{{ actualRange.end_s.toFixed(3) }} s</dd><dt>通道</dt><dd>{{ metric.channel ?? '' }}</dd><dt v-if="isDynamic">更新间隔</dt><dd v-if="isDynamic">{{ asRecord(metric.dynamic_contract).step_s ?? 1 }} s</dd></dl></section>
