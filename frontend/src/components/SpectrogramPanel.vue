...
<template>
  <section class="spectrogram-panel" aria-label="时频图">
    <header class="spectrum-header"><strong>时频图</strong><span v-if="result">{{ (result.actual_start_s ?? result.window_start_s).toFixed(2) }}–{{ (result.actual_end_s ?? result.window_start_s + result.window_duration_s).toFixed(2) }} s · {{ result.algorithm_version }} · dB µV²/Hz</span></header>
    <p v-if="loading && !result" class="spectrum-empty">正在计算时频图…</p><p v-else-if="error && !result" class="spectrum-error">{{ error }}</p>
    <template v-if="result"><div class="spectrum-controls"><label>模式 <select v-model="mode"><option value="static">静态时频图</option><option value="dynamic">动态时频图</option></select></label><label v-if="mode === 'dynamic'">窗口 <select v-model.number="dynamicWindow"><option :value="5">最近 5 s</option><option :value="10">最近 10 s</option><option :value="20">最近 20 s</option></select></label><label v-if="mode === 'dynamic'">刷新 <select v-model.number="refreshStep"><option :value="1">每 1 s</option><option :value="2">每 2 s</option><option :value="5">每 5 s</option></select></label><label>通道 <select :value="channel" @change="selected = ($event.target as HTMLSelectElement).value"><option v-for="name in result.channels" :key="name">{{ name }}</option></select></label><label>频段 <select v-model="selectedBand"><option value="all">全频 1–30 Hz</option><option value="delta">Delta 1–4 Hz</option><option value="theta">Theta 4–8 Hz</option><option value="alpha">Alpha 8–13 Hz</option><option value="beta">Beta 13–30 Hz</option><option value="custom">自定义范围</option></select></label><label v-if="selectedBand === 'custom'">起始 <input v-model.number="customMinHz" type="number" min="1" max="30" step="0.25" /></label><label v-if="selectedBand === 'custom'">结束 <input v-model.number="customMaxHz" type="number" min="1" max="30" step="0.25" /></label><label>显示 <select v-model="viewMode"><option value="trend">频段趋势</option><option value="heatmap">时频热图</option><option value="both">趋势 + 热图</option></select></label><button type="button" @click="reload">刷新时频图</button><button type="button" @click="algorithmOpen = true">算法校验</button><span v-if="loading" class="spectrum-status">正在更新</span></div><p class="spectrogram-reading-guide">{{ selectedBand === 'all' ? '频段趋势用于判断哪个波、什么时候变化；需要定位具体频率时选择热图。' : selectedBand === 'custom' ? `当前聚焦 ${frequencyRange.min}–${frequencyRange.max} Hz；趋势值由后端逐窗口积分，热图仍使用完整精度矩阵。` : `当前聚焦 ${selectedBand.toUpperCase()} 频段。后端仍保留完整频率矩阵。` }}</p><div class="spectrogram-band-guide"><span>Delta 1–4 Hz</span><span>Theta 4–8 Hz</span><span>Alpha 8–13 Hz</span><span>Beta 13–30 Hz</span></div><p v-if="result.quality" class="spectrogram-quality-summary">质量门：{{ result.quality.clean_windows }}/{{ result.quality.total_windows }} clean<span v-if="result.quality.bad_windows"> · {{ result.quality.bad_windows }} 个窗口已拒绝（灰色区域保留时间位置，悬停可查看原因）</span><span v-else> · 全部窗口通过</span></p><BandPowerTrendChart v-if="result.band_power_timeseries && viewMode !== 'heatmap'" :result="result" :channel="channel" :band="selectedBand" /><p v-if="error" class="spectrum-error">{{ error }}</p><SpectrogramChart v-if="viewMode !== 'trend'" :result="result" :channel="channel" :frequency-range="frequencyRange" /></template><p v-else-if="!loading && !error" class="spectrum-empty">暂无时频数据</p>
    <SpectrogramAlgorithmDialog v-if="algorithmOpen && result" :result="result" :channel="channel" @close="algorithmOpen = false" />
  </section>
</template>
