...
const staticResult = ref<Awaited<ReturnType<typeof getConfiguredSpectrogram>> | null>(null)
const dynamicHistory = ref<Awaited<ReturnType<typeof getConfiguredSpectrogram>> | null>(null)
...
const resultDisplayRange = ref(30)
const RESULT_DISPLAY_RANGE_OPTIONS = [10, 20, 30, 60] as const
const MAX_DYNAMIC_HISTORY_S = Math.max(...RESULT_DISPLAY_RANGE_OPTIONS)
const result = computed(() => mode.value === 'dynamic'
  ? (dynamicHistory.value ? visibleSpectrogramHistory(dynamicHistory.value, resultDisplayRange.value) : null)
  : staticResult.value)
...
<template>
  <section class="spectrogram-panel" aria-label="时频图">
    <header class="spectrum-header"><strong>时频图</strong><span v-if="result">实际分析范围 {{ (result.actual_start_s ?? result.window_start_s).toFixed(2) }}–{{ (result.actual_end_s ?? result.window_start_s + result.window_duration_s).toFixed(2) }} s<template v-if="mode === 'dynamic'"> · 结果展示范围最近 {{ resultDisplayRange }} s</template> · {{ result.algorithm_version }} · dB µV²/Hz</span></header>
    <p v-if="loading && !result" class="spectrum-empty">正在计算时频图…</p><p v-else-if="error && !result" class="spectrum-error">{{ error }}</p>
    <template v-if="result">
      <div class="spectrum-controls">
        <label>模式 <select v-model="mode"><option value="static">静态时频图</option><option value="dynamic">动态时频图</option></select></label>
        <label v-if="mode === 'dynamic'">分析范围 <select v-model.number="dynamicWindow"><option :value="5">最近 5 s</option><option :value="10">最近 10 s</option><option :value="20">最近 20 s</option></select></label>
        <label v-if="mode === 'dynamic'">更新间隔 <select v-model.number="refreshStep"><option :value="1">每 1 s</option><option :value="2">每 2 s</option><option :value="5">每 5 s</option></select></label>
        <label v-if="mode === 'dynamic'">结果展示范围 <select v-model.number="resultDisplayRange"><option v-for="seconds in RESULT_DISPLAY_RANGE_OPTIONS" :key="seconds" :value="seconds">最近 {{ seconds }} s</option></select></label>
        <label>通道 <select :value="channel" @change="selected = ($event.target as HTMLSelectElement).value"><option v-for="name in result.channels" :key="name">{{ name }}</option></select></label>
        <label>频段 <select v-model="selectedBand"><option value="all">全频 1–30 Hz</option><option value="delta">Delta 1–4 Hz</option><option value="theta">Theta 4–8 Hz</option><option value="alpha">Alpha 8–13 Hz</option><option value="beta">Beta 13–30 Hz</option><option value="custom">自定义范围</option></select></label>
        <label v-if="selectedBand === 'custom'">起始 <input v-model.number="customMinHz" type="number" min="1" max="30" step="0.25" /></label><label v-if="selectedBand === 'custom'">结束 <input v-model.number="customMaxHz" type="number" min="1" max="30" step="0.25" /></label>
      ...
      <p class="spectrogram-reading-guide">{{ selectedBand === 'all' ? '频段趋势用于判断哪个波、什么时候变化；需要定位具体频率时选择热图。' : selectedBand === 'custom' ? `当前聚焦 ${frequencyRange.min}–${frequencyRange.max} Hz；趋势值由后端逐个频谱计算片段积分，热图仍使用完整精度矩阵。` : `当前聚焦 ${selectedBand.toUpperCase()} 频段。后端仍保留完整频率矩阵。` }}</p>
      <div class="spectrogram-band-guide"><span>Delta 1–4 Hz</span><span>Theta 4–8 Hz</span><span>Alpha 8–13 Hz</span><span>Beta 13–30 Hz</span></div>
      <p v-if="result.quality" class="spectrogram-quality-summary">质量门：{{ result.quality.clean_windows }}/{{ result.quality.total_windows }} clean<span v-if="result.quality.bad_windows"> · {{ result.quality.bad_windows }} 个计算片段已拒绝（灰色区域保留时间位置，悬停可查看原因）</span><span v-else> · 全部计算片段通过</span></p>
      <BandPowerTrendChart v-if="result.band_power_timeseries && viewMode !== 'heatmap'" :result="result" :channel="channel" :band="selectedBand" /><p v-if="error" class="spectrum-error">{{ error }}</p><SpectrogramChart v-if="viewMode !== 'trend'" :result="result" :channel="channel" :frequency-range="frequencyRange" />
