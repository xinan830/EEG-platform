...
<template>
  <template v-for="extension in props.extensions" :key="extension.kind">
    ...
    <section v-else-if="extension.kind === 'faa_paired_quality'" class="algorithm-debug-section"><h3>FAA 成对 Epoch 质量</h3><p>来源：{{ Array.isArray(extension.data.channels) ? extension.data.channels.join(' / ') : '—' }} · Alpha：{{ range(extension.data.band) }}</p><div class="algorithm-debug-inputs"><div><strong>有效 Epoch</strong><span>{{ number(extension.data.clean_epochs, 0) }} / {{ number(extension.data.total_epochs, 0) }}</span></div><div><strong>质量比例</strong><span>{{ typeof extension.data.clean_ratio === 'number' ? `${(extension.data.clean_ratio * 100).toFixed(2)}%` : '—' }}</span></div><div><strong>拒绝原因</strong><span>{{ extension.data.reason || '无' }}</span></div></div></section>
  </template>
</template>
