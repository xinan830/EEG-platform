...
import { DEFAULT_DYNAMIC_METRIC_POLICY, appendOrRetainDynamicMetric, dynamicMetricBootstrapRange, dynamicMetricCatchupRange, playbackMetricWindow, type DynamicMetricPolicy, type DynamicWindowS } from '../utils/dynamicMetricPlayback'
import '../styles/algorithmDisplayWorkspace.css'

type Range = { start: number; end: number }
type Settings = { channel: string; mode: 'static' | 'dynamic'; startS: number; endS: number; windowS: DynamicWindowS; stepS: 1 | 5; displayRangeS: number }
type DynamicDefinition = { id: string; label: string; unit: string; channel: string; windowS: DynamicWindowS; stepS: 1 | 5; minimumWindowS: number; allowWarmup: boolean; displayRangeS: number }
...
function defaults(key: string): Settings { const policy = dynamicPolicy(key); return { channel: channels.value[0] ?? '', mode: 'static', startS: range.value.start, endS: range.value.end, windowS: policy.default_window_s as DynamicWindowS, stepS: policy.refresh_step_s as 1 | 5, displayRangeS: 30 } }
...
async function submit(key: string, startS: number, endS: number, dynamic: boolean, append = false) {
  ...
  try {
    ...
    const request = officialId ? { source: 'official' as const, recordingId: props.recording.id, algorithmId: officialId, channel: value.channel, startS, endS, mode: dynamic ? 'dynamic' as const : 'static' as const, dynamicWindowS: dynamic ? value.windowS : undefined, refreshStepS: dynamic ? value.stepS : undefined } : userRequest(key, value, startS, endS, dynamic)
...
async function refreshDynamic(position: number) { await Promise.all(keys.value.filter((key) => config(key).mode === 'dynamic').map(async (key) => { const policy = metricPolicy(key); const endpoint = scheduledEndpoint(position, policy.refreshStepS); if (isRunning(key) || refreshSeconds.value[key] === endpoint) return; const previous = refreshSeconds.value[key] ?? null; const window = previous === null ? playbackMetricWindow(endpoint, config(key).windowS, policy) : dynamicMetricCatchupRange(previous, endpoint, config(key).windowS, policy.refreshStepS); if (!window) return; refreshSeconds.value = { ...refreshSeconds.value, [key]: endpoint }; await submit(key, window.startS, window.endS, true, true) })) }
