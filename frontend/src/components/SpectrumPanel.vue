...
<template>
  <section class="spectrum-panel" aria-label="频谱分析">
    <header class="spectrum-header"><strong>PSD 与频段功率</strong><span v-if="result">{{ mode === 'dynamic' ? `动态频谱 · 分析范围最近 ${dynamicWindow} s · 每 ${refreshStep} s 更新` : '静态频谱 · 已提交分析范围' }} · 实际分析范围 {{ result.window_start_s.toFixed(2) }}–{{ (result.window_start_s + result.window_duration_s).toFixed(2) }} s</span></header>
    <template v-if="result">
      ...
      <div class="spectrum-controls"><label>分析方式 <select v-model="mode"><option value="static">静态频谱 · 稳定摘要</option><option value="dynamic">动态频谱 · 播放中更新</option></select></label><label v-if="mode === 'dynamic'">分析范围 <select v-model.number="dynamicWindow"><option :value="5">最近 5 s</option><option :value="10">最近 10 s</option><option :value="20">最近 20 s</option><option :value="30">最近 30 s</option></select></label><label v-if="mode === 'dynamic'">更新间隔 <select v-model.number="refreshStep"><option :value="1">每 1 s</option><option :value="2">每 2 s</option><option :value="5">每 5 s</option></select></label><label>查看通道 <select :value="firstChannel" @change="selectedChannel = ($event.target as HTMLSelectElement).value"><option v-for="channel in result.channels" :key="channel" :value="channel">{{ channel }}</option></select></label><button type="button" @click="reload">更新结果</button><button type="button" @click="algorithmOpen = true">计算与校验</button></div>
      <div v-if="mode === 'static'" class="spectrum-range-controls"><span>分析范围</span><label>开始 <input v-model.number="staticInputs.start" type="number" min="0" step="0.1"> s</label><label>结束 <input v-model.number="staticInputs.end" type="number" min="4" step="0.1"> s</label><button type="button" @click="applyStaticRange">分析该范围</button><button type="button" @click="useCurrentThirtySeconds">使用当前 30 秒</button><button type="button" @click="useCurrentScreen">使用波形显示范围</button></div>
      ...
      <small class="spectrum-meta">离线分析参考：源记录（不进行软件重参考） · PSD：{{ result.units.psd }} · 频谱计算片段：4 s Welch / 50% 重叠 · 有效片段 {{ result.quality.clean_segments }}/{{ result.quality.total_segments }}</small>
    </template>
