...
class RunService:
    ...
    def _execute_dynamic_definition_metric(self, recording: Any, version: Any, config: DefinitionMetricConfig):
        ...
        values: list[float] = []
        start = float(config.time.start_s)
        ...
        return {
            "metric": {
                "mode": "dynamic", "output": {"id": output_id, "label": output_label or output_id, "unit": output_unit},
                "channel": config.channel, "actual_range": {"start_s": start, "end_s": end},
                "dynamic_contract": {"window_s": config.dynamic_window_s, "step_s": config.refresh_step_s, "alignment": "window_end"},
                "series": points,
                "chart": {"kind": "metric_trend", "x_axis": {"label": "时间", "unit": "s", "field": "window_end_s"},
                          "y_axis": {"label": output_label or output_id, "unit": output_unit}},
            }
        }, {"metric_time_s": np.asarray([point["time_s"] for point in points], dtype=float),
            "metric_values": np.asarray(values, dtype=float)}, output_unit
