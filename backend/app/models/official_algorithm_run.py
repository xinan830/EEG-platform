...
class OfficialChannelMappingRequired(ValueError):
    """Official composite execution never infers semantic channel roles."""
...
class OfficialAlgorithmRunConfig(BaseModel):
    ...
    def validate_contract(self) -> "OfficialAlgorithmRunConfig":
        if self.algorithm_id == "iapf" and not self.channel:
            raise ValueError("IAPF requires one analysis channel")
