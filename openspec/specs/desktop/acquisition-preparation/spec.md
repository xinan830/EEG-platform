# desktop/acquisition-preparation Specification

## Purpose
This capability defines the explicit montage and channel-configuration inputs required before opening an EEG acquisition stream.
...
- **WHEN** the user selects a valid montage and sampling rate and starts acquisition
- **THEN** the desktop SHALL validate the current device against the montage's channel snapshot
- **AND** it SHALL open the stream with that snapshot's channel labels and hardware REF/GND locations
- **AND** it SHALL persist both channel and montage snapshots in stream metadata.
...
- **AND** no global acquisition mapping SHALL change until an acquisition preparation session selects a montage.
...
- **WHEN** the selected montage is average, specified-pair, or channel referenced
- **THEN** the display trace SHALL be computed from the retained raw channel values
